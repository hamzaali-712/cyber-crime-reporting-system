...
==============================
Three classes, each with full validation + Firebase persistence:

  Class 1 – PersonalInformation
      → Firestore  : collection  'personal_information'  (doc ID = CNIC)
      → RTDB       : node  'complaints/{cnic}/status'   (live status tracking)

  Class 2 – IncidentDetails
      → Firestore  : collection  'incident_details'     (auto doc ID)
      → RTDB       : node  'complaints/{cnic}/incident' (live updates)

  Class 3 – EvidenceUpload
      → Firestore  : collection  'evidence_uploads'     (metadata)
      → Storage    : path  'evidence/{incident_id}/{filename}'
"""
...
from firebase_config import get_firestore, get_rtdb, get_storage_bucket

# Lazy clients (resolved on first use)
_fs      = None   # Firestore
_rtdb    = None   # Realtime Database root ref
_bucket  = None   # Storage bucket
...
class PersonalInformation:
    ...
    Firestore  →  personal_information/{cnic}
                  { full_name, cnic, gender, mobile, email, occupation, created_at }

    RTDB       →  complaints/{cnic}/status
                  { status: "Submitted", updated_at: "..." }
                  (enables live complaint-status tracking on the frontend)

    Validation rules
    ----------------
    full_name   : required · 2–100 chars
    cnic        : required · format 12345-1234567-1 · must be unique
    gender      : required · Male | Female | Other
    mobile      : required · starts with +92 · exactly 10 digits after code
    email       : optional · must contain @ when provided
    occupation  : optional
    """
    ...
    def validate(self) -> dict:
        ...
        if self.gender not in self.GENDERS:
            return _err("gender",
                        f"Gender must be one of: {', '.join(sorted(self.GENDERS))}.")

        # Mobile – Pakistan only, +92 prefix, exactly 10 digits after code
        if not self.mobile:
        ...
        if not self.MOBILE_RE.match(self.mobile):
            return _err("mobile",
                        ...
                        "(no spaces, letters, or symbols).  Example: +923001234567")

        # Email – optional but needs @
    ...
    def _cnic_exists(self) -> bool:
        doc = _firestore().collection(self.FS_COLLECTION).document(self.cnic).get()
        return doc.exists

    def save(self) -> dict:
        """
        1. Validate fields.
        2. Check CNIC uniqueness in Firestore.
        3. Write personal data to Firestore.
        4. Create live-status node in Realtime Database.
        """
        ...
        _firestore().collection(self.FS_COLLECTION).document(self.cnic).set(fs_payload)

        # ── RTDB: complaints/{cnic}/status ───────────────────────────────────
        _realtime().child(self.RTDB_NODE).child(self.cnic).child("status").set({
            "status"     : "Submitted",
            "updated_at" : _now(),
        })
    ...
    def get_by_id(cnic: str) -> dict | None:
        """Fetch from Firestore by CNIC."""
        doc = _firestore().collection(
            PersonalInformation.FS_COLLECTION
        ).document(cnic).get()
        return doc.to_dict() if doc.exists else None

    @staticmethod
    def get_status(cnic: str) -> dict | None:
        """Fetch live status from Realtime Database."""
        return _realtime().child(
            PersonalInformation.RTDB_NODE
        ).child(cnic).child("status").get()

    @staticmethod
    def update_status(cnic: str, new_status: str) -> dict:
        """Update complaint status in RTDB (admin use)."""
        if new_status not in COMPLAINT_STATUSES:
            return _err("status",
                        f"Invalid status. Choose from: {', '.join(COMPLAINT_STATUSES)}")
        _realtime().child(
            PersonalInformation.RTDB_NODE
        ).child(cnic).child("status").update({
            "status"     : new_status,
            "updated_at" : _now(),
        })
...
class IncidentDetails:
    ...
    Firestore  →  incident_details/{auto_id}
                  { personal_id, postal_address, city, crime_category,
                    crime_details, created_at }

    RTDB       →  complaints/{cnic}/incident
                  { incident_id, city, crime_category, created_at }
                  (lightweight live snapshot for dashboards)

    Validation rules
    ----------------
    city            : required · must be a valid Pakistani city
    crime_category  : required · must be from predefined list
    ...
    def save(self) -> dict:
        """
        1. Validate fields.
        2. Verify parent personal record exists in Firestore.
        3. Write incident to Firestore (auto ID).
        4. Mirror lightweight snapshot to RTDB.
        """
        result = self.validate()
        if not result["success"]:
            return result

        # ── Verify parent personal record ────────────────────────────────────
        parent = _firestore().collection(
            PersonalInformation.FS_COLLECTION
        ).document(self.personal_id).get()
        ...
        _, doc_ref = _firestore().collection(self.FS_COLLECTION).add(fs_payload)
        incident_id = doc_ref.id

        # ── RTDB: complaints/{cnic}/incident ─────────────────────────────────
        _realtime().child(self.RTDB_NODE).child(self.personal_id).child("incident").set({
    ...
    def get_by_id(incident_id: str) -> dict | None:
        doc = _firestore().collection(
            IncidentDetails.FS_COLLECTION
        ).document(incident_id).get()
        if not doc.exists:
            return None
        data = doc.to_dict()
    ...
    def get_by_personal_id(personal_id: str) -> list[dict]:
        docs = (
            _firestore().collection(IncidentDetails.FS_COLLECTION)
...
class EvidenceUpload:
    ...
    Storage    →  evidence/{incident_id}/{timestamp}_{filename}
                  (actual file bytes – images & videos)

    Firestore  →  evidence_uploads/{auto_id}
                  { incident_id, file_name, file_type, storage_path,
                    public_url, uploaded_at }

    Accepted types  : PNG, JPG/JPEG · MP4, MKV
    Max total size  : 500 MB  (enforced in the API layer)
    Upload is fully optional – skipping it does not block a complaint.
    """
    ...
    def __init__(self, incident_id: str, file_name: str,
        ...
        self.file_name   = Path(file_name).name          # strip any path traversal
    ...
    def save(self) -> dict:
        """
        1. Validate the file.
        2. Verify parent incident exists in Firestore.
        3. Upload file bytes to Firebase Storage.
        ...
        # ── Verify parent incident ───────────────────────────────────────────
        parent = _firestore().collection(
            IncidentDetails.FS_COLLECTION
        ).document(self.incident_id).get()
        ...
        storage_path = (
            f"{self.STORAGE_BASE}/{self.incident_id}/{timestamp}_{self.file_name}"
        )

        blob = _storage().blob(storage_path)
        content_type = (
    ...
    def get_by_incident(incident_id: str) -> list[dict]:
        docs = (
            _firestore().collection(EvidenceUpload.FS_COLLECTION)
